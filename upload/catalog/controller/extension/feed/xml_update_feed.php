...
class ControllerExtensionFeedXmlUpdateFeed extends Controller {
    ...
    public function updateStockAndPrices()
    {
        ...
        $exist_codes  = Product::pluck('sku')->flatten();
        ...
        $exist_codes  = ProductOption::pluck('sku')->flatten();
        $loaded       = $xml->collect()->whereIn('Sku', $exist_codes);
        $temp_product = '';
    ...
    public function updateSpecials()
    {
        ...
        $products   = Product::whereIn('sku', collect($specials)->pluck('Sku'))->get();
        $temp_query = '';
        ...
        // OPTIONS SPECIALS
        $options    = ProductOption::whereIn('sku', collect($specials)->pluck('Sku'))->get();
        $temp_query = '';
        $temp_products = [];
        ...
        foreach ($temp_products as $key => $sku) {
            $sale_price = $collection->where('Sku', $sku)->pluck('SalePrice');

            $temp_query .= '("' . $key . '", 1, 1, ' . (string)$sale_price . ', "0000-00-00", "0000-00-00"),';
        }
